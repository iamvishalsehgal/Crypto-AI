...
class OnChainCollector:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self._alchemy_key: str = settings.onchain.alchemy_api_key
        ...
        if self._session is None or self._session.closed:
            self._session = aiohttp.ClientSession(
                timeout=aiohttp.ClientTimeout(
                    total=self._settings.onchain.http_timeout_sec
                ),
            )
    ...
    async def fetch_whale_transfers(
        ...
        if min_value_usd is None:
            min_value_usd = self._settings.onchain.whale_min_value_usd
    ...
    async def _fetch_whale_alchemy(
        ...
        try:
            ...
            for tx in raw_transfers:
                ...
                eth_price = self._settings.onchain.eth_price_estimate
                if value >= min_value_usd / max(eth_price, 1.0):
                    transfers.append(
                        {
                            "hash": tx.get("hash"),
                            "from": tx.get("from"),
    ...
    async def _fetch_whale_etherscan(
        ...
        try:
            ...
            for tx in data.get("result", []):
                ...
                if value_eth >= min_value_usd / max(eth_price, 1.0):
                    transfers.append(
                        {
                            "hash": tx.get("hash"),
                            "from": tx.get("from"),
