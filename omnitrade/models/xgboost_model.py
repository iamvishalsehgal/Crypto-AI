...
class XGBoostTrader:
    ...
    def train(
        ...
        logger.info(
            "Training XGBoost: %d train, %d val, %d features (class weights: %s)",
            len(y_train_mapped),
            len(y_val_mapped),
    ...
    def predict(self, X: Union[np.ndarray, pd.DataFrame]) -> np.ndarray:
        ...
        ------
        RuntimeError
            If the model has not been trained or loaded yet, or if input
        ...
        if not np.isfinite(X).all():
            raise RuntimeError("XGBoostTrader.predict received NaN or Inf values")
