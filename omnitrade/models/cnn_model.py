...
Usage::
    ...
    from omnitrade.models.cnn_model import (
...
class CandlestickCNN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if torch.isnan(x).any() or torch.isinf(x).any():
            raise RuntimeError("CNN input contains NaN or Inf values")
        x = self.features(x)
        x = self.classifier(x)
        return x
...
class CNNTrainer:
    ...
    def predict_proba(self, images: np.ndarray) -> np.ndarray:
        ...
        if not np.isfinite(images).all():
            raise RuntimeError("CNN predict received NaN or Inf values in images")
