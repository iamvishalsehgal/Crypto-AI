...
class LSTMPredictor(nn.Module):
    ...
    *cell_type* and *bidirectional* flags.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Raises:
            RuntimeError: If input contains NaN or Inf values.
        """
        if torch.isnan(x).any() or torch.isinf(x).any():
            raise RuntimeError("LSTM input contains NaN or Inf values")
...
class LSTMTrainer:
    ...
    def prepare_sequences(
        ...
        X_train = self.scaler.transform(X_train_flat).reshape(n_train, seq_len, n_feats)

        n_val = X_val_raw.shape[0]
        X_val_flat = X_val_raw.reshape(-1, n_feats)
        X_val = self.scaler.transform(X_val_flat).reshape(n_val, seq_len, n_feats)
