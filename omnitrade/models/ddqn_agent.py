...
Usage::
    ...
    from omnitrade.models.ddqn_agent import DDQNAgent
    ...
    * **target_network** -- used to evaluate Q-values for the DDQN target and
    ...
    def replay(self) -> Optional[float]:
        ...
        if torch.isnan(loss) or torch.isinf(loss):
            logger.error("DDQN loss is NaN/Inf — skipping batch")
            return None
        ...
        # Detect NaN gradients post-step
        for name, param in self.policy_network.named_parameters():
            if param.grad is not None and (torch.isnan(param.grad).any() or torch.isinf(param.grad).any()):
                logger.error("NaN/Inf gradient in %s — resetting optimizer", name)
                self.optimizer.zero_grad()
                return None
    ...
    def train(
        ...
        Args:
            env: A Gymnasium-compatible training environment.
            episodes: Number of training episodes.
            max_steps: Optional cap on steps per episode.
            val_env: Optional validation environment for early stopping.
                If provided, evaluated every ``checkpoint_freq`` episodes.
            early_stopping_patience: Stop if no val improvement after N evals.
            checkpoint_dir: Directory to save best model checkpoint.
            checkpoint_freq: Evaluate (and possibly save) every N episodes.

        Returns:
            Dictionary with ``'episode_rewards'``, ``'episode_lengths'``,
            ``'epsilon_values'``, ``'losses'``, ``'val_rewards'``,
            ``'best_episode'``, ``'early_stopped'``.
        """
        ...
        logger.info(
            "Starting DDQN training: %d episodes, val=%s, patience=%d",
            episodes,
            val_env is not None,
        ...
        for episode in range(1, episodes + 1):
            ...
            if (
                val_env is not None
                and episode % checkpoint_freq == 0
            ):
                val_reward = self._evaluate(val_env, n_episodes=3)
                history["val_rewards"].append(val_reward)
                history["val_episodes"].append(episode)
                ...
                if val_reward > best_val_reward:
                    ...
                    if checkpoint_dir is not None:
                        ckpt_path = Path(checkpoint_dir) / "ddqn_best.pth"
                        self.save_model(ckpt_path)
                        logger.info(
                            "New best val reward %.2f at episode %d — saved to %s",
                            val_reward, episode, ckpt_path,
                        )
                else:
                ...
                if episodes_without_improvement >= early_stopping_patience:
                    logger.info(
                        "Early stopping at episode %d: no improvement for %d evals",
                        episode, episodes_without_improvement,
                    )
                    early_stopped = True
                    break
        ...
        logger.info(
            "DDQN training complete. Best train: %+.2f, best val: %+.2f (ep %d), "
            "early_stopped=%s",
            best_reward, best_val_reward, best_episode, early_stopped,
        )
    ...
    def _evaluate(self, env: Any, n_episodes: int = 3) -> float:
