...
class HealthChecker:
    ...
    # ------------------------------------------------------------------
    # Health endpoint response (HTTP-style: 200 OK / 503 Service Unavailable)
    # ------------------------------------------------------------------

    def get_health_response(
        self,
        lane_status: Optional[Dict[str, str]] = None,
    ) -> Dict[str, Any]:
        """Return an HTTP-compatible health check response.

        Designed for use by a lightweight health endpoint or orchestrator
        (e.g. Docker HEALTHCHECK, Kubernetes liveness probe, GitHub Actions).

        Args:
            lane_status: Optional per-lane status dict from AssetRouter,
                e.g. ``{"crypto": "active", "stock": "error: ..."}``.

        Returns:
            Dict with ``http_status`` (200 or 503), ``overall``, ``lanes``,
            ``system``, and ``timestamp``.
        """
        system = self.check_system_resources_sync()
        lanes = lane_status or {}

        degraded_lanes = [
            k for k, v in lanes.items()
            if v != "active" and not v.startswith("error")
        ]
        failed_lanes = [k for k, v in lanes.items() if v.startswith("error")]
        ...
        if failed_lanes:
            ...
            overall = "degraded"
            http_status = 200
        elif system.get("disk_percent", 0) > _DISK_WARN:
            overall = "degraded"
            http_status = 200
        elif system.get("memory_percent", 0) > _MEMORY_WARN:
            overall = "degraded"
            http_status = 200
        else:
            overall = "healthy"
        ...
        return {
            ...
            "lanes": {
                "total": len(lanes),
                "active": sum(1 for v in lanes.values() if v == "active"),
                "degraded": degraded_lanes,
                "failed": failed_lanes,
                "details": lanes,
            },
            "system": system,
    ...
    @staticmethod
    def check_system_resources_sync() -> Dict[str, float]:
        """Synchronous system resource check (for health endpoints)."""
        try:
            import psutil
            return {
