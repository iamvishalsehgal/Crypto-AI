...
class EnsembleVoter:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self._max_consecutive_failures: int = getattr(
            getattr(settings, "safety", None), "kill_on_consecutive_losses", 5
        )
    ...
    def vote(self, features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        *features* dict; plain callables are called with *features*
        ...
        # If ALL registered models have exceeded max failures, halt.
        if self._models and all(
            self._failure_counts.get(n, 0) >= self._max_consecutive_failures
            for n in self._models
        ):
            raise RuntimeError(
                f"All {len(self._models)} models have failed "
                f"{self._max_consecutive_failures}+ consecutive times — halting"
            )
    ...
    def degraded(self) -> bool:
        """True if any model has consecutive failures at or above threshold."""
        return any(
            c >= self._max_consecutive_failures
            for c in self._failure_counts.values()
        )

    def get_failure_status(self) -> Dict[str, Any]:
        """Return per-model failure counts and overall health.

        Returns:
            Dict with ``healthy`` (bool), ``degraded_count`` (int),
            and ``models`` mapping model name to failure count.
        """
        return {
            "healthy": not self.degraded,
            "degraded_count": sum(
                1 for c in self._failure_counts.values()
                if c >= self._max_consecutive_failures
            ),
            "models": dict(self._failure_counts),
            "max_failures": self._max_consecutive_failures,
        }

    def reset_failures(self) -> None:
        """Reset all failure counters (e.g. after model reload)."""
        self._failure_counts.clear()
        logger.info("Ensemble failure counters reset")
