...
class SafetyGuard:
    ...
    def __init__(self, settings: Optional[Settings] = None) -> None:
        ...
        # ── Rolling volatility history for dynamic thresholds ──
        self._atr_ratio_history: deque = deque(maxlen=500)
        self._bb_bandwidth_history: deque = deque(maxlen=500)
        self._min_samples_for_dynamic: int = 30  # need at least 30 obs for reliable percentile
    ...
    def _check_volatility(self, features: Dict) -> SafetyVerdict:
        ...
        # Track ATR ratio for dynamic threshold computation
        if atr is not None and close is not None and close > 0:
            atr_pct = atr / close
            self._atr_ratio_history.append(atr_pct)

            if len(self._atr_ratio_history) >= self._min_samples_for_dynamic:
                extreme_thresh = float(np.percentile(self._atr_ratio_history, 95))
                elevated_thresh = float(np.percentile(self._atr_ratio_history, 80))
            else:
                extreme_thresh = 0.05
                elevated_thresh = 0.03

            if atr_pct > extreme_thresh:
                return SafetyVerdict(
                    False,
                    f"Extreme volatility: ATR {atr_pct:.2%} > {extreme_thresh:.2%} "
                    f"(95th percentile) — sitting out",
                )
            if atr_pct > elevated_thresh:
                warnings.append(
                    f"High volatility: ATR {atr_pct:.2%} > {elevated_thresh:.2%} "
                    f"(80th percentile)"
                )

        # Track BB bandwidth for dynamic threshold
        if bb_bandwidth is not None:
            self._bb_bandwidth_history.append(bb_bandwidth)

            if len(self._bb_bandwidth_history) >= self._min_samples_for_dynamic:
                bb_thresh = float(np.percentile(self._bb_bandwidth_history, 90))
            else:
                bb_thresh = 0.15

            if bb_bandwidth > bb_thresh:
                warnings.append(
                    f"Wide Bollinger bands: {bb_bandwidth:.3f} > {bb_thresh:.3f} "
                    f"(90th percentile)"
                )
