...
from omnitrade.config.logging_config import setup_logging


def get_logger(name: str = "omnitrade") -> logging.Logger:
    """
    Return a configured logger for the given *name*.

    On the first call the root project logger is initialised via
    :func:`omnitrade.config.logging_config.setup_logging`.  Subsequent calls
